	...
	public class ActiveTimeRange
	{
		...
		public string ActiveFromTime
		{
			...
			set
			{
				...
				{
					throw new ArgumentException(string.Format(InfrastructureResources.InvalidActiveFromTime, value));
				}
				else
				{
		...
		public string ActiveToTime
		{
			...
			set
			{
				...
				if (!DateTime.TryParseExact(value, "HH:mm", CultureInfo.InvariantCulture, DateTimeStyles.None, out dt))
				{
					throw new ArgumentException(string.Format(InfrastructureResources.InvalidActiveToTime, value));
				}
				else
				{
